...
final class PipelineRunner {
    ...
    /// The record currently represented by the result screen. Keeping this
    /// separate from `assets` lets recovery paths retain stable sheet context.
    private(set) var resultRecord: MeetingRecord?
    private var pendingReviewRecord: MeetingRecord?
    private var pendingReviewMaterials: [SupportingMaterial] = []

    var issueReviewWorkspaceID: UUID? {
        pendingReviewRecord?.workspaceID ?? assets?.workspace?.id
    }

    var resultTitle: String {
        assets?.title ?? resultRecord?.title ?? pendingReviewRecord?.title ?? "会议纪要"
    }

    var resultWorkspaceID: UUID? {
        assets?.workspace?.id ?? resultRecord?.workspaceID ?? pendingReviewRecord?.workspaceID
    }

    /// A generic association failure puts every issue into manual review. It
    /// is safe to retry only this final model call from the persisted draft.
    var canRetryIssueAssociation: Bool {
        issueReviewReasons.contains {
            $0.contains("问题关联未完成") || $0.contains("问题关联重试失败")
        }
    }
    ...
    func restoreIssueReview(_ draft: PendingIssueReviewDraft,
        ...
        // A restored issue-review draft used to populate only the rendered
        // summary. Result actions then had no assets and SwiftUI presented an
        // empty, non-dismissible sheet. Rebuild the lightweight assets that
        // survive persistence so export, learning and feedback remain usable.
        assets = Self.recoveredAssets(from: record, workspace: workspace,
                                      materials: materials)
        resultRecord = record
    ...
    /// Re-runs only the independent issue-association node. The completed
    /// transcript, screen review and minutes generation are reused verbatim.
    func retryIssueAssociation() {
        guard canRetryIssueAssociation,
              var record = pendingReviewRecord,
              var structured = record.structuredSummary,
        ...
        // The fallback path may have inserted conservative local tracking IDs.
        // The retry must start from clean model-generated issues so a `null`
        // model decision cannot accidentally retain a fallback association.
        IssuePreflight.resetAssociationsForRetry(in: &structured)
        let materials = pendingReviewMaterials
        let context = TokenUsageContext(
            feature: "重新关联会议问题", customer: record.customerName,
            project: record.projectName, meetingID: record.meetingGroupID ?? record.id,
            meetingTitle: record.title)

        task = Task { [weak self] in
            guard let self else { return }
            do {
                let association = try await TokenUsageContext.$current.withValue(context) {
                    try await IssueAssociationService(settings: Settings.shared)
                        .associate(structured, workspaceID: record.workspaceID)
                }
                var associated = association.minutes
                let confirmedIssues = record.workspaceID.map {
                    (try? ProjectLedgerStore.load().issues(for: $0)) ?? []
                } ?? []
                IssueTracking.prepare(&associated, confirmedIssues: confirmedIssues)
                record.structuredSummary = associated
                record.summaryMarkdown = StructuredMinutesRenderer.markdown(from: associated)
                if association.phase.calls > 0 {
                    if record.generationUsage == nil {
                        record.generationUsage = GenerationUsage(phases: [], isEstimated: true)
                    }
                    record.generationUsage?.phases.append(association.phase)
                }

                self.pendingReviewRecord = record
                self.structuredSummary = associated
                self.summary = record.summaryMarkdown
                let reviewRisks = IssuePreflight.risks(in: associated.issues)
                    + association.reviewReasons
                self.issueReviewReasons = reviewRisks

                if Settings.shared.alwaysReviewIssues || !reviewRisks.isEmpty {
                    try PendingIssueReviewStore.save(.init(
                        record: record, materialPaths: materials.map { $0.sourceURL.path },
                        reasons: reviewRisks))
                    self.stage = .reviewingIssues
                    self.progress = 1
                    self.detail = reviewRisks.isEmpty
                        ? "问题关联已完成，请确认后生成最终纪要"
                        : "问题关联已完成，仍有少量风险需要确认"
                } else if self.persist(record, materials: materials) {
                    PendingIssueReviewStore.clear()
                    self.pendingReviewRecord = nil
                    self.pendingReviewMaterials = []
                    self.issueReviewReasons = []
                    self.stage = .done
                    self.progress = 1
                    self.detail = "问题关联完成，纪要已保存"
                    self.emitRunSummary(status: "成功（问题关联重试完成）")
                } else {
                    self.stage = .reviewingIssues
                    self.progress = 1
                    self.detail = "问题关联已完成，但纪要保存失败，请重试确认。"
                }
            } catch is CancellationError {
                self.stage = .reviewingIssues
                self.progress = 1
                self.detail = "已取消问题关联重试"
            } catch {
                let reason = "问题关联重试失败：\(error.localizedDescription)"
                self.issueReviewReasons = [reason]
                self.stage = .reviewingIssues
                self.progress = 1
                self.detail = reason
                try? PendingIssueReviewStore.save(.init(
                    record: record, materialPaths: materials.map { $0.sourceURL.path },
                    reasons: [reason]))
                self.debugSession?.nodeWarning("问题关联", detail: reason)
            }
            self.isRunning = false
            PipelineDebugRegistry.install(nil)
        }
    ...
    func analyzeEditedTranscript(record: MeetingRecord, transcript: Transcript,
        ...
        usedSummaryFallback = false; isRunning = true; savedRecordID = nil
        resultRecord = nil
        var editedRecord = record
        editedRecord.summaryMarkdown = ""
        let bundle = Self.recoveredAssets(from: editedRecord, workspace: workspace,
                                          materials: materials, transcript: transcript)
        assets = bundle
        task = Task { [weak self] in
            guard let self else { return }
    ...
    /// Saves identity metadata and learned voice profiles without paying for a
    /// new minutes generation. The already-rendered minutes stay unchanged;
    /// future meetings receive the updated directory defaults.
    func saveSpeakerMetadata(_ names: [Int: String], roles: [Int: SpeakerRole]) {
        guard var bundle = assets, var diarization = bundle.diarization else { return }
        diarization.names.merge(names) { _, new in new }
        diarization.roles.merge(roles.filter { $0.value.isSpecified }) { _, new in new }
        bundle.diarization = diarization
        assets = bundle
        guard let id = savedRecordID else { return }
        do {
            resultRecord = try MeetingHistoryStore.updateSpeakers(
                id: id, names: diarization.names, roles: diarization.roles)
            historyWarning = nil
        } catch {
            historyWarning = "说话人信息保存失败：\(error.localizedDescription)"
        }
    }

    func applyRecognitionMemoryAndRegenerate() {
        guard !isRunning else { return }
        if var bundle = assets {
        ...
        // Defensive fallback for legacy/incomplete recovery state. The dialog
        // can still save learning rules and regenerate from the persisted
        // transcript instead of becoming a dead action.
        guard let record = resultRecord ?? pendingReviewRecord else { return }
        let workspace = MeetingWorkspaceStore.resolve(
            id: record.workspaceID, customerName: record.customerName,
            projectName: record.projectName)
        let transcript = RecognitionMemoryStore.apply(
            to: record.transcript, workspaceID: workspace?.id ?? record.workspaceID)
        analyzeEditedTranscript(record: record, transcript: transcript,
                                workspace: workspace, materials: pendingReviewMaterials)
    }

    private static func recoveredAssets(
        ...
        MeetingAssets(
            ...
            transcript: transcript ?? record.transcript, captures: [], hasVideo: false,
    ...
    private func execute(url: URL, title: String?, meetingContext: String,
        ...
        if wantsSpeakers, !forceRetranscribe, !forceSpeakerSeparation, let speakerKey,
           var cached = DiarizationCache.load(key: speakerKey) {
            ...
            let voiceMatches = VoiceProfileStore.matchReport(
                embeddings: cached.embeddings,
                qualities: cached.embeddingQualities,
                workspaceID: workspace?.id)
            cached.names = voiceMatches.names
            cached.nameSuggestions = voiceMatches.suggestions
            cached.roles = PersonRoleDirectory.defaults(
                names: cached.names, workspace: workspace)
            diarization = cached
            usedCachedDiarization = true
            let metadata = DiarizationCache.metadata(key: speakerKey)
            debugSession?.cache("声纹分离", hit: true,
                                detail: "缓存键：\(speakerKey)\n文件：\(metadata?.path ?? "<未知>")\n生成时间：\(metadata?.createdAt.map { ISO8601DateFormatter().string(from: $0) } ?? "<未知>")\n大小：\(metadata?.bytes ?? 0) 字节\n说话片段：\(cached.segments.count)\n\(cached.clusterDiagnosticsDescription)\n跳过引擎：声纹模型\n匹配结果：\n\(voiceMatches.logDescription)")
        ...
        if transcript == nil, let audioURL {
            ...
            识别记忆选择：
            \(RecognitionMemoryStore.promptSelectionDescription(
                workspaceID: workspace?.id, context: learningContext))
            """)
            ...
            try Task.checkCancellation()
            let correction = RecognitionMemoryStore.applyDetailed(
                to: raw, workspaceID: workspace?.id)
            let fresh = correction.transcript
            ...
            if let debug = debugSession {
                ...
                本地纠错：\(correction.logDescription)
                文本：\(textURL?.path ?? "<写入失败>")
        ...
        if wantsSpeakers, diarization == nil, let audioURL {
            ...
            do {
                ...
                自动聚类阈值：\(speakerCount == 0 ? String(format: "%.2f", Diarizer.automaticClusteringThreshold) : "固定人数模式")
                """)
                ...
                let voiceMatches = VoiceProfileStore.matchReport(
                    ...
                    qualities: result.embeddingQualities,
                    workspaceID: workspace?.id)
                result.names = voiceMatches.names
                result.nameSuggestions = voiceMatches.suggestions
                result.roles = PersonRoleDirectory.defaults(
                    names: result.names, workspace: workspace)
                ...
                \(result.clusterDiagnosticsDescription)
                匹配结果：
                \(voiceMatches.logDescription)
                """)
            } catch {
        ...
        if var identified = diarization {
            ...
            identified.roles.merge(speakerRoles.filter { $0.value.isSpecified }) {
                _, saved in saved
            }
    ...
    static func transcriptionPrompt(scenario: RecognitionScenario,
        ...
        let components = [priorityVocabulary, glossary, materialContext, scenario.transcriptionHint]
            .map(cleanWhisperPromptComponent)
            .filter { !$0.isEmpty }
        var seen = Set<String>()
        return components.flatMap { component in
            component.components(separatedBy: CharacterSet(charactersIn: "，、,"))
        }.compactMap { fragment in
            let value = fragment.trimmingCharacters(in: .whitespacesAndNewlines)
            let key = value.folding(options: [.caseInsensitive, .diacriticInsensitive],
                                    locale: .current)
