...
struct PromptBuilder {
    ...
    若标记还包含人工确认的所属方与角色（如【张三｜客户｜领导】），用角色判断其授权范围：\
    客户领导的明确决定、否决、优先级和验收口径应重点保留；项目经理的计划、依赖和协调\
    事项应转成可跟踪内容；Sales 的转述不得冒充客户确认；工程师的技术结论仍需证据支持。\
    **角色不是事实正确性的通用权重**：领导的推测仍是推测，工程师对商务授权的判断也不是决定。
    ...
    12. **问题必须能独立跟踪和闭环。** 日志不足、证据不足、排查困难、验证环境缺失、\
    待补充材料等通常是原问题的排查限制或方案，不得单独拆成问题。处于同一故障链的\
    “现象—诊断限制—排查方案”应合并；只有跟踪对象、状态或闭环条件独立时才拆分。\
    不同告警类型、产品对象、根因或处置路径不得仅因都属于“告警噪声”“同一议题”而合并，\
    例如暴力破解告警与 USB 告警应分别输出。输出前自检：若某条脱离另一问题后无法独立描述\
    或闭环，将其并入对应问题；若合并后的任一部分需要独立负责人、方案或闭环判断，则必须拆开。
...

    {
      ...
      "issues": [{
        ...
        "background": "问题出现的业务或技术背景", "rootCause": "根因",
        "solution": "方案", "progress": "本次会议新增或确认的进展，不重复堆砌完整历史",
        "evidence": ["[12:34]"]
      }],
      "requirements": [{
      ...
      "actionItems": [{
        ...
        "issueID": "该待办明确服务于某个本次问题时，填写 issues 中完全一致的 title；独立待办填 null",
    ...
    private func assembleTimeline(includingScreenText: Bool) -> String {
        ...
        for entry in entries {
            ...
            case .speech(let segment):
                ...
                if let diarization = assets.diarization {
                    if let speaker = diarization.speaker(from: segment.start, to: segment.end) {
                        lastImportedSpeaker = nil
                        ...
                        // Attribution continuity is broken. Force the next attributed line to
                        // print its name even if it belongs to the previously seen speaker.
                        lastSpeaker = nil
