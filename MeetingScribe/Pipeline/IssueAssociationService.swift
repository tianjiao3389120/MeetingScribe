...
struct IssueAssociationService {
    ...
    判断问题身份时，优先看稳定的受影响对象、验证目标、闭环标准和前后会议的连续性。
    如果仍在推进同一对象的同一项测试、复测或投产验证，即使本次新增了第三方、资源、排期或环境阻塞，
    也应作为原问题的最新阻塞和进展，不要仅因即时根因或阻塞来源变化就新建问题。
    仅当本次描述了可独立解决、独立关闭的另一故障或工作目标时才拆分；只有产品名相同并不足以关联。
    标题相似但对象、独立闭环目标或处置完全不同时，不得关联。只输出 JSON。
    ...
    func associate(_ minutes: StructuredMinutes, workspaceID: UUID?) async throws -> Result {
        ...
        let localRanking = selection.rankingLines.joined(separator: "\n")
        let user = """
        本次问题（issueIndex 为下面的数字）：
        \(current)

        本地相关度排名（只用于确保逐项比较，不代表必须关联）：
        \(localRanking.isEmpty ? "<无>" : localRanking)
        ...
        for index in updated.issues.indices {
            ...
            guard let id = match.trackingID else {
                if let exact = ranked.first(where: {
                    ...
                    let open = ranked.filter { !$0.issue.isClosed }
                    if let first = open.first {
                        let secondScore = open.dropFirst().first?.score ?? 0
                        let strong = first.score >= 20
                            && (secondScore == 0 || first.score >= secondScore * 1.35)
                        if strong {
                            reviewReasons.append(
                                "“\(updated.issues[index].title)”高度相关于历史问题“\(first.issue.title)”但模型判为新问题")
                        }
                    }
                }
                continue
