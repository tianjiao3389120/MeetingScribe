...
enum IssuePreflight {
    static func resetAssociationsForRetry(in minutes: inout StructuredMinutes) {
        for index in minutes.issues.indices {
            minutes.issues[index].trackingID = nil
            clearProposedProfile(in: &minutes.issues[index])
        }
    }
