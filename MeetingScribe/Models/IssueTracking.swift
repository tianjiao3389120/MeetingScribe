...
enum IssueTracking {
    ...
    /// Uses the same local relevance model as automatic association, so a
    /// person can search with current-meeting wording instead of remembering
    /// the exact historical title.
    static func manualSearch(from issues: [ProjectIssue], evidenceText: String,
                             includeUnmatched: Bool = false) -> [CandidateScore] {
        let query = evidenceText.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !query.isEmpty else {
            return issues.sorted { $0.updatedAt > $1.updatedAt }.map {
                CandidateScore(issue: $0, score: 0, reason: "最近更新")
            }
        }
        let ranked = rankedIssues(issues, evidenceText: query)
        return includeUnmatched ? ranked : ranked.filter {
            $0.score > 0 || $0.reason == "标题/别名命中"
        }
    }
