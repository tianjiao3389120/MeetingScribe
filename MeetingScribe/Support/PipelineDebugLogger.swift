...
final class PipelineDebugSession: @unchecked Sendable {
    ...
    private static let retainedRunLimit = 20

    init(root: URL? = nil, pausesAtNodeStart: Bool) throws {
        let base = root ?? AppEnvironment.supportDirectory
            .appendingPathComponent("Debug", isDirectory: true)
        try FileManager.default.createDirectory(
            at: base, withIntermediateDirectories: true,
            attributes: [.posixPermissions: 0o700])
        Self.pruneRuns(in: base)
    ...
    func runSummary(status: String, cache: String, screen: String, extra: String = "",
                    node: String = "流水线", tokenOverride: Int? = nil) {
        lock.lock()
        ...
        lock.unlock()
        let tokenLines = tokenOverride.map { "Token：\($0)" }
            ?? (tokens.isEmpty ? "Token：<不可用>" : tokens.sorted { $0.key < $1.key }
                .map { "Token · \($0.key)：\($0.value)" }.joined(separator: "\n"))
        let total = tokens.values.reduce(0, +)
        let totalText = tokenOverride.map(String.init)
            ?? (total > 0 ? String(total) : "<不可用>")
        write(type: "RUN SUMMARY", node: node, detail: """
        状态：\(status)
    ...
    private static func pruneRuns(in base: URL, now: Date = Date()) {
        ...
        guard let children = try? manager.contentsOfDirectory(
            at: base, includingPropertiesForKeys: Array(keys), options: [.skipsHiddenFiles]) else {
            return
        }
        let runs: [(url: URL, date: Date)] = children.compactMap { url in
            guard url.lastPathComponent.hasPrefix("run-") else { return nil }
        ...
        where index >= retainedRunLimit || run.date < cutoff {
