...
enum LibraryBackup {
    ...
    struct Paths {
        ...
        var voiceRecognitionMetrics: URL { root.appendingPathComponent("voice-recognition-metrics.json") }
    ...
    static func restoreArchive(from archive: URL, destination: Paths = .live) throws -> RestoreResult {
        ...
        catch { throw Failure.invalidBackup }
    ...
    static func createPackage(at package: URL, source: Paths) throws {
        ...
        for (from, name) in [(source.workspaces, "workspaces.json"),
                             ...
                             (source.voiceRecognitionMetrics, "voice-recognition-metrics.json"),
    ...
    static func restorePackage(from package: URL, destination: Paths) throws -> RestoreResult {
        ...
        if manager.fileExists(atPath: voiceSource.path) {
            ...
            let ids = Set(existing.map(\.id))
            let identities = Set(existing.map {
                VoiceProfileStore.identityKey(name: $0.name, workspaceID: $0.workspaceID)
            })
            let additions = incoming.filter {
                !ids.contains($0.id)
                    && !identities.contains(VoiceProfileStore.identityKey(
                        name: $0.name, workspaceID: $0.workspaceID))
            }
        ...
        let metricsSource = package.appendingPathComponent("voice-recognition-metrics.json")
        if manager.fileExists(atPath: metricsSource.path),
           !manager.fileExists(atPath: destination.voiceRecognitionMetrics.path) {
            try manager.copyItem(at: metricsSource, to: destination.voiceRecognitionMetrics)
            try manager.setAttributes([.posixPermissions: 0o600],
                                      ofItemAtPath: destination.voiceRecognitionMetrics.path)
            filesAdded += 1
        }
