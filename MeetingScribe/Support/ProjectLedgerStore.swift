...
enum ProjectLedgerStore {
    enum Failure: LocalizedError {
        case proposalNotFound, actionNotFound, duplicateIssueID, duplicateIssueTitle
        var errorDescription: String? {
            switch self {
            case .proposalNotFound: "找不到这条项目更新建议。"
            case .actionNotFound: "建议关联的项目行动项不存在。"
            case .duplicateIssueID: "项目问题 ID 已存在，无法重复创建。"
            case .duplicateIssueTitle: "同一项目中已存在同名问题，请使用其他名称。"
            }
        }
    }
    ...
    static func acceptIssue(proposalID: UUID, targetIssueID: String? = nil,
        ...
        case .update:
            ...
            // An issue's canonical title is its stable identity across meetings. Keep the
            // wording generated for this meeting on the timeline and as a searchable alias,
            // rather than renaming the formal issue every time it is mentioned differently.
            if ledger.issues[index].title != proposal.title {
                ledger.issues[index].aliases = mergedTerms(
                    ledger.issues[index].aliases, [proposal.title])
            }
            ledger.issues[index].aliases = mergedTerms(
    ...
    @discardableResult
    static func renameIssue(issueID: String, title: String,
                            at url: URL = fileURL) throws -> ProjectLedger {
        var ledger = try load(from: url)
        guard let index = ledger.issues.firstIndex(where: { $0.id == issueID })
        else { throw Failure.actionNotFound }
        ...
        guard !value.isEmpty else { return ledger }
        let issue = ledger.issues[index]
        guard issue.title != value else { return ledger }
        let normalized = value.folding(options: [.caseInsensitive, .diacriticInsensitive],
                                       locale: .current)
        guard !ledger.issues.contains(where: {
            $0.id != issueID && $0.workspaceID == issue.workspaceID
                && $0.title.folding(options: [.caseInsensitive, .diacriticInsensitive],
                                    locale: .current) == normalized
        }) else { throw Failure.duplicateIssueTitle }

        let now = Date()
        ledger.issues[index].aliases = mergedTerms(issue.aliases, [issue.title])
            .filter {
                $0.folding(options: [.caseInsensitive, .diacriticInsensitive], locale: .current)
                    != normalized
            }
        ledger.issues[index].title = value
        ledger.issues[index].updatedAt = now
        ledger.issues[index].events.append(ProjectIssueEvent(
            kind: .updated, occurredAt: now, meetingID: issue.sourceMeetingID,
            meetingTitle: "手动修改问题名称", previousStatus: issue.status,
            currentStatus: issue.status, title: value, background: issue.background,
            rootCause: issue.rootCause, solution: issue.solution,
            progress: "问题名称由“\(issue.title)”修改为“\(value)”", evidence: []))
        try save(ledger, to: url)
        return ledger
    }

    @discardableResult
    static func setIssueStar(issueID: String, starred: Bool,
                             at url: URL = fileURL) throws -> ProjectLedger {
        var ledger = try load(from: url)
        guard let index = ledger.issues.firstIndex(where: { $0.id == issueID })
        else { throw Failure.actionNotFound }
        guard ledger.issues[index].isStarred != starred else { return ledger }
        // Priority is display metadata: do not alter updatedAt, add a timeline event,
        // or make a previously generated issue analysis stale.
        ledger.issues[index].starredAt = starred ? Date() : nil
        try save(ledger, to: url)
        return ledger
    }
