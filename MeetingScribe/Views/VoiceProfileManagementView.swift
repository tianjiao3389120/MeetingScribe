...
struct VoiceProfileManagementView: View {
    ...
    @State private var metrics = VoiceRecognitionMetrics()
    ...
    private func profileSummary(_ profile: VoiceProfile) -> String {
        var value = "\(profile.sampleCount) 次确认 · \(profile.representativeEmbeddings.count) 个代表声纹"
        if profile.quarantinedSampleCount > 0 {
            value += " · 已隔离 \(profile.quarantinedSampleCount) 条异常样本"
        }
        if profile.pendingEnvironmentCount > 0 {
            value += " · 新环境待再次确认"
        }
        return "\(value) · 更新于 \(profile.updatedAt.formatted(date: .abbreviated, time: .omitted))"
    }

    private func load() {
        do {
            profiles = try VoiceProfileStore.loadChecked()
            metrics = VoiceRecognitionMetricsStore.load()
            error = nil
        } catch {
    ...
    private var metricsSummary: String {
        let precision = metrics.automaticPrecision.map {
            String(format: "%.0f%%", $0 * 100)
        } ?? "暂无"
        return "本地验证：\(metrics.reviewedMeetings) 场会议 · 自动识别人工确认率 \(precision)（正确 \(metrics.automaticConfirmed) / 修正 \(metrics.automaticCorrected)）· 候选采纳 \(metrics.suggestionsAccepted)/\(metrics.suggestionsPresented)"
    }
