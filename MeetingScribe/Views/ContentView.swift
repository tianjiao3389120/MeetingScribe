...
struct ContentView: View {
    ...
    @State private var environmentStartupError = UserDefaults.standard.string(
        forKey: AppEnvironment.startupErrorKey)
    ...
    var body: some View {
        ...
        .alert("正式环境初始化失败", isPresented: Binding(
            get: { environmentStartupError != nil },
            set: { if !$0 { environmentStartupError = nil } }
        )) {
            Button("好") { environmentStartupError = nil }
        } message: {
            Text(environmentStartupError ?? "未知错误")
        }
    }
                ...
                VStack(alignment: .leading, spacing: 5) {
                    HStack(spacing: 10) {
                        ...
                        Text(AppEnvironment.label)
                            ...
                            .foregroundStyle(AppEnvironment.isProduction ? .green : .orange)
                            .padding(.horizontal, 8).padding(.vertical, 4)
                            .background(
                                (AppEnvironment.isProduction ? Color.green : Color.orange)
...
private struct IssuePreflightView: View {
    ...
    init(runner: PipelineRunner) {
        ...
        if let workspaceID = runner.issueReviewWorkspaceID {
        ...
        VStack(spacing: 0) {
            ...
            HStack {
                Text("需确认 \(reviewIndices.count) 个 · 自动确认 \(automaticDrafts.count) 个")
                    .font(.caption).foregroundStyle(.secondary)
                Spacer()
                if runner.canRetryIssueAssociation {
                    Button("重新尝试 AI 问题关联") {
                        runner.retryIssueAssociation()
                    }
                    .buttonStyle(.bordered)
                    .help("只重新运行问题关联，不会重新转录、抽帧或生成纪要")
                }
                            ...
                            if !historicalIssues.isEmpty && !draft.wrappedValue.excluded {
                                Picker("历史关联", selection: Binding(
                                    ...
                                    ForEach(historicalIssues) { issue in
                                        Text("\(issue.title) · \(issue.isClosed ? "已闭环" : (issue.status.isEmpty ? "待确认" : issue.status))")
...
private struct SavedMeetingSheetContext: Identifiable {
    let id: UUID
    let title: String
}
...
private struct ResultView: View {
    ...
    @State private var feedbackContext: SavedMeetingSheetContext?
    @State private var minutesVersionsContext: SavedMeetingSheetContext?
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack(spacing: 12) {
                ...
                if let id = runner.savedRecordID {
                    Button("会议纪要") {
                        minutesVersionsContext = .init(id: id, title: runner.resultTitle)
                    }
                }
                ...
                Menu("更多", systemImage: "ellipsis.circle") {
                    ...
                    if runner.savedRecordID != nil {
                        ...
                        Button("反馈并改进纪要…") {
                            if let id = runner.savedRecordID {
                                feedbackContext = .init(id: id, title: runner.resultTitle)
                            }
                        }
                    }
                    Divider()
        ...
        .sheet(isPresented: $showNaming) {
            if let assets = runner.assets {
                SpeakerNamingView(assets: assets) { names, roles, regenerate in
                    showNaming = false
                    if regenerate {
                        runner.applySpeakerNamesAndRegenerate(names, roles: roles)
                    } else {
                        runner.saveSpeakerMetadata(names, roles: roles)
                    }
                } onCancel: {
                    showNaming = false
                }
        ...
        .sheet(item: $feedbackContext) { context in
            MeetingFeedbackView(meetingID: context.id, title: context.title) { request in
                runner.regenerate(withFeedback: request)
            }
        }
        .sheet(isPresented: $showRecognitionLearning) {
            RecognitionLearningView(
                workspaceID: runner.resultWorkspaceID,
                sourceTitle: runner.resultTitle,
                uncertainties: runner.structuredSummary?.uncertainties ?? [],
                currentTranscript: runner.assets?.transcript.plainText
                    ?? runner.resultRecord?.transcript.plainText ?? "") { regenerate in
                    if regenerate { runner.applyRecognitionMemoryAndRegenerate() }
                }
        }
        .sheet(item: $minutesVersionsContext) { context in
