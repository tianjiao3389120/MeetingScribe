...
struct WorkspaceDashboardView: View {
    ...
    @State private var editingIssueTitle: ProjectIssue?
    ...
    private var issues: [ProjectIssue] {
        ledger.issues(for: workspace.id).sorted {
            if $0.isStarred != $1.isStarred { return $0.isStarred }
            if $0.isStarred, $0.starredAt != $1.starredAt {
                return ($0.starredAt ?? .distantPast) > ($1.starredAt ?? .distantPast)
            }
    ...
    var body: some View {
        ...
        .sheet(item: $editingIssueTitle) { issue in
            IssueTitleEditorView(issue: issue) { title in
                renameIssue(issue, title: title)
                editingIssueTitle = nil
            } onCancel: { editingIssueTitle = nil }
        }
    ...
    private func projectIssueRow(_ issue: ProjectIssue) -> some View {
        DisclosureGroup {
            ...
            HStack {
                Button {
                    setIssueStar(issue, starred: !issue.isStarred)
                } label: {
                    Image(systemName: issue.isStarred ? "star.fill" : "star")
                        .foregroundStyle(issue.isStarred ? Color.yellow : Color.secondary)
                }
                .buttonStyle(.plain)
                .help(issue.isStarred ? "取消重点问题" : "标为重点问题")
                .accessibilityLabel(issue.isStarred ? "取消重点问题" : "标为重点问题")
                VStack(alignment: .leading, spacing: 3) {
                ...
                Button(analysis == nil ? "一键分析" :
                        (analysis?.needsAnalysisRefresh(comparedWith: issue) == true
                         ? "重新分析" : "查看分析")) {
                    analyzingIssue = issue
                }
                    .buttonStyle(.borderless).controlSize(.small)
                    .help(analysis?.needsAnalysisRefresh(comparedWith: issue) == true
                          ? "问题有新进展或分析规则已升级，需要重新分析"
                          : "基于已确认关联的会议和证据生成问题专题分析")
                Button(issue.isClosed ? "重新打开" : "修改状态") {
                    editingIssueStatus = issue
                }
                .buttonStyle(.borderless).controlSize(.small)
                Button("修改名称") { editingIssueTitle = issue }
                    .buttonStyle(.borderless).controlSize(.small)
            }
        }
    ...
    private func renameIssue(_ issue: ProjectIssue, title: String) {
        do {
            ledger = try ProjectLedgerStore.renameIssue(issueID: issue.id, title: title)
            ledgerError = nil
        } catch { ledgerError = "问题名称修改失败：\(error.localizedDescription)" }
    }

    private func setIssueStar(_ issue: ProjectIssue, starred: Bool) {
        do {
            ledger = try ProjectLedgerStore.setIssueStar(
                issueID: issue.id, starred: starred)
            ledgerError = nil
        } catch { ledgerError = "重点标记更新失败：\(error.localizedDescription)" }
    }
...
private struct IssueTitleEditorView: View {
    let issue: ProjectIssue
    let onSave: (String) -> Void
    let onCancel: () -> Void
    @State private var title: String

    init(issue: ProjectIssue, onSave: @escaping (String) -> Void,
         onCancel: @escaping () -> Void) {
        self.issue = issue; self.onSave = onSave; self.onCancel = onCancel
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("修改问题名称").font(.title2.weight(.semibold))
            TextField("问题名称", text: $title)
                .textFieldStyle(.roundedBorder)
            Text("原名称会保留为搜索别名；已有会议记录、状态和问题分析不会丢失。")
                .font(.caption).foregroundStyle(.secondary)
            HStack {
                Spacer()
                Button("取消", action: onCancel)
                Button("保存") { onSave(cleanedTitle) }
                    .buttonStyle(.borderedProminent)
                    .keyboardShortcut(.defaultAction)
                    .disabled(cleanedTitle.isEmpty || cleanedTitle == issue.title)
            }
        }
        .padding(20)
        .frame(width: 480)
...
private struct IssueAssociationEditorView: View {
    ...
    private var searchEvidence: String {
        let query = searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        if !query.isEmpty { return query }
        return [proposal.title, proposal.background, proposal.rootCause,
                proposal.solution, proposal.progress].joined(separator: "\n")
    }

    private var rankedIssues: [IssueTracking.CandidateScore] {
        IssueTracking.manualSearch(
            from: issues, evidenceText: searchEvidence,
            includeUnmatched: showAllIssues)
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack {
                TextField("搜索已有问题：可输入关键词或本次问题描述", text: $searchText)
                    .textFieldStyle(.roundedBorder)
                Button(showAllIssues ? "只看相关" : "显示全部") {
            ...
            List(filteredIssues, selection: $selectedIssueID) { issue in
                VStack(alignment: .leading, spacing: 3) {
                    HStack {
                        Text(issue.title)
                        Spacer()
                        if let rank = rankedIssues.firstIndex(where: { $0.issue.id == issue.id }),
                           rank < 3 {
                            Text(rank == 0 ? "最相关" : "相关 \(rank + 1)")
                                .font(.caption2.weight(.medium))
                                .foregroundStyle(rank == 0 ? Color.accentColor : .secondary)
                        }
                    }
                    Text("\(issue.isClosed ? "已闭环" : (issue.status.isEmpty ? "待确认" : issue.status)) · \(issue.id)")
                        .font(.caption2)
                        .foregroundStyle(issue.isClosed ? Color.green : .secondary)
                }
