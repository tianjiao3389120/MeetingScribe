...
struct StorageManagementView: View {
    ...
    @State private var confirmDebugRemoval = false
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Form {
                ...
                Section("备份与恢复") {
                    ...
                    Text("备份包含会议、托管材料、会议空间、反馈和声纹档案；不包含缓存、识别引擎或 API Key。备份为未加密 ZIP，请仅保存在受信任的加密位置。恢复只合并新项目，不覆盖现有数据。")
                        .font(.caption).foregroundStyle(.secondary)
                }
                Section("自动备份") {
                ...
                Section("调试数据") {
                    LabeledContent("隐藏调试日志") {
                        Text("\(overview.debugRunCount) 次 · \(bytes(overview.debugBytes))")
                    }
                    Button("清除调试日志", role: .destructive) {
                        confirmDebugRemoval = true
                    }.disabled(overview.debugRunCount == 0)
                    Text("日志可能包含完整逐字稿、提示词、模型输出和文件路径；清除不影响会议资料库。")
                        .font(.caption).foregroundStyle(.secondary)
                }
                Section("说话人运行环境") {
        ...
        .alert("清除调试日志？", isPresented: $confirmDebugRemoval) {
            Button("取消", role: .cancel) {}
            Button("清除", role: .destructive) {
                let directory = AppEnvironment.supportDirectory
                    .appendingPathComponent("Debug", isDirectory: true)
                try? FileManager.default.removeItem(at: directory)
                refresh()
            }
        } message: {
            Text("将删除本环境的调试日志和中间产物，会议记录、缓存和声纹档案不受影响。")
        }
