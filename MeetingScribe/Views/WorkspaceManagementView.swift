...
struct WorkspaceManagementView: View {
    ...
    private var companyDetail: some View {
        VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(spacing: 10) {
                    ForEach(ourProfiles) { item in
                        ...
                        HStack(spacing: 12) {
                            ...
                            Picker("默认角色", selection: profile.meetingRole) {
                                ForEach(SpeakerRole.MeetingRole.allCases) {
                                    Text($0.label).tag($0)
                                }
                            }
                            .labelsHidden()
                            .frame(width: 110)
                            Spacer()
                            Text(profileSummary(profile.wrappedValue))
            ...
            Text("人员归属和默认角色会在后续会议识别出姓名时自动带入；会议中仍可临时覆盖。")
                .font(.caption).foregroundStyle(.secondary)
        }
    }
    ...
    private func profileSummary(_ profile: VoiceProfile) -> String {
        let base = "\(profile.representativeEmbeddings.count) 个代表声纹 · 已确认 \(profile.sampleCount) 次"
        var value = profile.quarantinedSampleCount > 0
            ? "\(base) · 已隔离 \(profile.quarantinedSampleCount) 条异常样本" : base
        if profile.pendingEnvironmentCount > 0 { value += " · 新环境待再次确认" }
        return value
    }
