            ...
            Form {
                ...
                Section("高级诊断") {
                    DisclosureGroup("展开诊断选项") {
                        ...
                        Text("调试信息和中间产物仅供当前用户读取，保留最近 20 次且最长 14 天。目录：\(AppEnvironment.supportDirectory.appendingPathComponent("Debug").path)。终端查看：tail -f \"\(AppEnvironment.supportDirectory.appendingPathComponent("Debug/latest.log").path)\"")
                            .font(.caption).foregroundStyle(.secondary)
                    }
                }
            }
...
final class ModelDownloader {
    ...
    private func fetch(_ urlString: String,
        ...
        if FileManager.default.fileExists(atPath: destination.path) {
            detail = "正在校验\(label)…"
            if FileIntegrity.matchesSHA256(expectedSHA256, at: destination) { return }
