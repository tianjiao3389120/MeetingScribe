...
final class PipelineDebugLoggerTests: XCTestCase {
    func testDebugRunsUsePrivateDirectoryAndPruneExpiredData() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let expired = root.appendingPathComponent("run-expired", isDirectory: true)
    ...
    func testRunSummarySupportsStandaloneFeatureNode() throws {
        let root = temporaryRoot()
        defer { try? FileManager.default.removeItem(at: root) }
        let session = try PipelineDebugSession(root: root, pausesAtNodeStart: false)

        session.runSummary(status: "成功", cache: "不适用", screen: "不适用",
                           node: "问题专题分析")

        let log = try String(contentsOf: session.logURL, encoding: .utf8)
        XCTAssertTrue(log.contains("[RUN SUMMARY · 问题专题分析]"))
    }
