...
final class MeetingHistoryStoreTests: XCTestCase {
    ...
    func testSaveLoadExportAndDeleteWithoutSourceMedia() throws {
        ...
        let updated = try MeetingHistoryStore.updateClassification(
            ...
            workspaceID: workspaceID, customerName: "星河银行",
            projectName: "终端安全项目", tags: ["双周会", "客户"], root: root)
        XCTAssertEqual(updated.title, "客户项目周会")
        XCTAssertEqual(updated.workspaceID, workspaceID)
        XCTAssertEqual(updated.customerName, "星河银行")
        XCTAssertEqual(updated.projectName, "终端安全项目")
        ...
        XCTAssertEqual(updatedSibling.customerName, "星河银行")
        XCTAssertEqual(updatedSibling.projectName, "终端安全项目")
    ...
    @MainActor
    func testRestoredIssueReviewRebuildsResultContext() {
        let transcript = Transcript(segments: [
            .init(id: 0, start: 0, end: 3, speaker: "Taylor", text: "测试恢复")
        ])
        let record = MeetingRecord(
            title: "恢复后的会议", sourcePath: "/tmp/recovered.mov", duration: 3,
            backend: "测试", model: "mock", summaryMarkdown: "# 纪要",
            structuredSummary: nil, transcript: transcript,
            speakerNames: [:], usedSummaryFallback: false)
        let draft = PendingIssueReviewDraft(
            record: record, materialPaths: [], reasons: ["问题关联未完成"])
        let runner = PipelineRunner()

        runner.restoreIssueReview(draft, materials: [])

        XCTAssertEqual(runner.assets?.title, "恢复后的会议")
        XCTAssertEqual(runner.assets?.transcript.timecodedText, transcript.timecodedText)
        XCTAssertEqual(runner.resultTitle, "恢复后的会议")
        XCTAssertEqual(runner.stage, .reviewingIssues)
    }
    ...
    func testLibrarySortingAndTimeSections() throws {
        ...
        XCTAssertEqual(alphabetic[0].records.map(\.title), ["A-较早会议", "B-昨天会议", "C-今天会议"])
    }
