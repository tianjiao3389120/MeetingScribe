...
final class WorkspaceMaterialTests: XCTestCase {
    ...
    func testPromptTimelineRepeatsSpeakerAfterUnattributedGap() {
        let transcript = Transcript(segments: [
            TranscriptSegment(id: 0, start: 0, end: 1, text: "第一句"),
            TranscriptSegment(id: 1, start: 2, end: 3, text: "无法归属"),
            TranscriptSegment(id: 2, start: 4, end: 5, text: "第三句")
        ])
        let diarization = Diarization(
            segments: [SpeakerSegment(start: 0, end: 1, speaker: 0),
                       SpeakerSegment(start: 4, end: 5, speaker: 0)],
            names: [0: "张三"])
        let assets = MeetingAssets(sourceURL: URL(fileURLWithPath: "/tmp/test.wav"),
                                   duration: 5, transcript: transcript, captures: [],
                                   hasVideo: false, diarization: diarization)

        let timeline = PromptBuilder(assets: assets, contextHint: "", imageCaptureIDs: [])
            .buildTimeline()

        XCTAssertEqual(timeline.components(separatedBy: "【张三】").count - 1, 2)
        XCTAssertTrue(timeline.contains("[00:02] 无法归属"))
    }
    ...
    func testIssueStarPersistsWithoutChangingBusinessTimeline() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("project-issue-star-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: url) }
        let updatedAt = Date(timeIntervalSince1970: 100)
        let issue = ProjectIssue(
            id: "MS-ISSUE-STAR", workspaceID: UUID(), title: "重点问题", aliases: [],
            background: "", rootCause: "", solution: "", status: "进行中",
            createdAt: updatedAt, updatedAt: updatedAt, sourceMeetingID: UUID(), events: [])
        try ProjectLedgerStore.save(ProjectLedger(issues: [issue]), to: url)

        var ledger = try ProjectLedgerStore.setIssueStar(
            issueID: issue.id, starred: true, at: url)

        XCTAssertTrue(ledger.issues[0].isStarred)
        XCTAssertEqual(ledger.issues[0].updatedAt, updatedAt)
        XCTAssertTrue(ledger.issues[0].events.isEmpty)
        ledger = try ProjectLedgerStore.setIssueStar(
            issueID: issue.id, starred: false, at: url)
        XCTAssertFalse(ledger.issues[0].isStarred)
    }
    ...
    func testIssueProposalCanBeManuallyAssociatedWithExistingIssue() throws {
        ...
        XCTAssertEqual(ledger.issues[0].title, "供应商引擎异常")
        XCTAssertTrue(ledger.issues[0].aliases.contains("引擎主动退出"))
        XCTAssertEqual(ledger.issues[0].events.last?.title, "引擎主动退出")
    ...
    func testManualIssueSearchFindsHistoricalTitleFromCurrentMeetingWording() {
        var cdm = candidateIssue(index: 1, title: "CDM运行缓慢及新版本验证受阻")
        cdm.background = "CDM需在UAT完成新版本复测后判断能否投产"
        cdm.searchTerms = ["CDM测试", "UAT复测", "新版本安装包"]
        let unrelated = candidateIssue(index: 2, title: "WebShell哈希批量加白")

        let results = IssueTracking.manualSearch(
            from: [unrelated, cdm],
            evidenceText: "CDM测试受Microsoft外部问题阻塞")

        XCTAssertEqual(results.first?.issue.id, cdm.id)
        XCTAssertGreaterThan(results.first?.score ?? 0, 0)
    }

    func testIssueAssociationPromptTreatsChangedBlockerAsContinuousValidation() {
        XCTAssertTrue(IssueAssociationService.systemPrompt.contains("同一项测试、复测或投产验证"))
        XCTAssertTrue(IssueAssociationService.systemPrompt.contains("阻塞来源变化"))
        XCTAssertTrue(IssueAssociationService.systemPrompt.contains("可独立解决、独立关闭"))
    }
    ...
    func testIssueCanBeManuallyRenamedWithoutLosingItsPreviousSearchName() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("manual-issue-title-\(UUID().uuidString).json")
        defer { try? FileManager.default.removeItem(at: url) }
        let issue = ProjectIssue(
            id: "MS-ISSUE-TITLE", workspaceID: UUID(), title: "旧问题名称", aliases: ["历史别名"],
            background: "背景", rootCause: "根因", solution: "方案", status: "进行中",
            createdAt: Date(timeIntervalSince1970: 100), updatedAt: Date(timeIntervalSince1970: 100),
            sourceMeetingID: UUID(), events: [])
        try ProjectLedgerStore.save(ProjectLedger(issues: [issue]), to: url)

        let ledger = try ProjectLedgerStore.renameIssue(
            issueID: issue.id, title: "  新问题名称  ", at: url)

        XCTAssertEqual(ledger.issues[0].title, "新问题名称")
        XCTAssertEqual(Set(ledger.issues[0].aliases), Set(["历史别名", "旧问题名称"]))
        XCTAssertEqual(ledger.issues[0].events.last?.meetingTitle, "手动修改问题名称")
        XCTAssertTrue(ledger.issues[0].events.last?.progress.contains("旧问题名称") == true)
    }
    ...
    func testLearnedAndManualVocabularySurviveWhisperPromptLimit() {
        let legacy = String(repeating: "通", count: 155) + "星盾AI"
        let prompt = PipelineRunner.transcriptionPrompt(
            scenario: .hongKongMixed,
            priorityVocabulary: "(星盾诶)应识别为(星盾AI)",
            glossary: legacy,
            materials: [])
        let trimmed = Transcriber.trimGlossary(prompt)

        XCTAssertTrue(trimmed.hasPrefix("(星盾诶)应识别为(星盾AI)"))
        XCTAssertTrue(trimmed.contains("星盾AI"))
    ...
    func testWhisperPromptRemovesMarkdownInstructionsAndInlineComments() {
        let prompt = PipelineRunner.transcriptionPrompt(
            scenario: .mandarin,
            priorityVocabulary: "星盾诶应识别为星盾AI # 领域词表说明\n# 不应发送给模型",
            glossary: "Agent\n# 使用说明",
            materials: [])

        XCTAssertTrue(prompt.contains("星盾诶应识别为星盾AI"))
    ...
    @MainActor
    func testWhisperPromptDeduplicatesTermsAcrossMemoryAndManualGlossary() {
        let prompt = PipelineRunner.transcriptionPrompt(
            scenario: .mandarin,
            priorityVocabulary: "Taylor，星盾AI，星盾AI",
            glossary: "Taylor\nAgent\nAgent",
            materials: [])

        XCTAssertEqual(prompt, "Taylor，星盾AI，Agent")
    }
