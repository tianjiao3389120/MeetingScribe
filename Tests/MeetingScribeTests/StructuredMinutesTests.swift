...
final class StructuredMinutesTests: XCTestCase {
    func testAssociationRetryClearsFallbackLinksAndProfiles() {
        var minutes = StructuredMinutes(
            title: "周会", nature: "周会", duration: "30 分钟", agenda: [],
            participantAssessment: [], issues: [
                .init(trackingID: "ISSUE-FALLBACK", rollingSummary: "旧摘要",
                      proposedAliases: ["旧称"], proposedSearchTerms: ["旧词"],
                      proposedNegativeTerms: ["排除词"], title: "本次问题", status: "进行中",
                      rootCause: "", solution: "", progress: "", evidence: [])
            ], requirements: [], actionItems: [], agreements: [], afterMeeting: [],
            uncertainties: [])

        IssuePreflight.resetAssociationsForRetry(in: &minutes)

        XCTAssertNil(minutes.issues[0].trackingID)
        XCTAssertNil(minutes.issues[0].rollingSummary)
        XCTAssertNil(minutes.issues[0].proposedAliases)
        XCTAssertNil(minutes.issues[0].proposedSearchTerms)
        XCTAssertNil(minutes.issues[0].proposedNegativeTerms)
    }
    ...
    func testGeneratedMeetingTitleReplacesOnlyDefaultFilename() {
        ...
        XCTAssertEqual(MeetingTitleResolver.resolve(
            ...
            generatedTitle: "星河银行终端安全阶段进展双周会"),
            "星河银行终端安全阶段进展双周会")
        XCTAssertEqual(MeetingTitleResolver.resolve(
    ...
    func testHistoricalTitleUsesExistingStructuredTitleWithoutOverwritingManualName() {
        let minutes = StructuredMinutes(
            title: "星河银行终端安全阶段工作同步", nature: "双周会", duration: "30 分钟",
