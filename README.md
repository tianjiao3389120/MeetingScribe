...
实时字幕已从主流程下架；既有实时字幕记录仍可从“设置 → 存储”查看和导出。
...
生成纪要、双栏释义、同步邮件、香港繁体转换、历史待办回溯和旧字幕翻译统一使用
...
Claude Code 同样复用本机登录；两个 CLI 后端均以文字方式接收 OCR 结果。OpenAI Realtime
语音识别使用独立凭据，不受此处文本模型选择影响。
...
- 同一会议空间的新会议会参考历史未完成待办；任务明确对应、状态变化且带本次证据时间码时生成更新建议，确认后才修改历史台账
- 可在资料库按会议空间手动回溯历史待办，补齐功能启用前或曾经遗漏的状态建议；未提及的待办不会被推断为完成
...
还可以创建完整 ZIP 备份并合并恢复会议、托管材料、会议空间、反馈、实时字幕和声纹档案；
