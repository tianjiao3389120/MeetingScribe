...
识别质量可选择 `gpt-realtime-whisper` 的“实时优先”或 `gpt-4o-transcribe` 的“准确优先”；本场识别
提示可以临时补充客户、人名、产品名、英文缩写和会议主题，不会写入全局词表。实时分段使用服务端
VAD，以约 800ms 静音判断一句话结束，减少固定时间切分造成的断句；BlackHole 音频通过系统
`AVAudioConverter` 高质量重采样到 24kHz，转换失败时才回退到原线性算法。
